...
class ObjectTable {
 public:
  ...
  object_ref<T> LookupObject(X_HANDLE handle) {
    ...
    if (object && object->type() != T::kObjectType) {
      REXLOG_WARN("ObjectTable::LookupObject type mismatch: handle={:#010x} actual_type={} expected_type={}",
                  handle, static_cast<int>(object->type()), static_cast<int>(T::kObjectType));
      return object_ref<T>(nullptr);
    }
