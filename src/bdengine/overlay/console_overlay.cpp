/**
 * @file        bdengine/overlay/console_overlay.cpp
 *
 ...
 */
#include "bdengine/overlay/console_overlay.h"
#include <rex/cvar.h>
#include <imgui.h>
#include <algorithm>
#include <string>
...
static constexpr const char* kCategoryNames[] = {
    "core", "cpu", "apu", "gpu", "krnl", "fs"
};
static constexpr int kNumCategories = sizeof(kCategoryNames) / sizeof(kCategoryNames[0]);

static ImVec4 LevelColor(spdlog::level::level_enum level) {
    switch (level) {
        case spdlog::level::trace:    return {0.5f, 0.5f, 0.5f, 1.0f};  // grey
        case spdlog::level::debug:    return {0.4f, 0.9f, 0.9f, 1.0f};  // cyan
        case spdlog::level::info:     return {1.0f, 1.0f, 1.0f, 1.0f};  // white
        case spdlog::level::warn:     return {1.0f, 1.0f, 0.0f, 1.0f};  // yellow
        case spdlog::level::err:      return {1.0f, 0.4f, 0.4f, 1.0f};  // red
        case spdlog::level::critical: return {1.0f, 0.0f, 0.0f, 1.0f};  // bright red
        default:                      return {1.0f, 1.0f, 1.0f, 1.0f};
    }
}

ConsoleDialog::ConsoleDialog(rex::ui::ImGuiDrawer* imgui_drawer,
...
int ConsoleDialog::InputTextCallback(ImGuiInputTextCallbackData* data) {
    ...
    if (data->EventFlag == ImGuiInputTextFlags_CallbackHistory) {
        ...
        if (prev != self->history_pos_) {
            const char* hist = (self->history_pos_ >= 0)
                ? self->history_[self->history_pos_].c_str() : "";
...
void ConsoleDialog::ExecuteCommand(std::string_view cmd) {
    ...
    if (cmd == "help" || cmd == "?") {
        ...
        for (auto& n : names) {
            ...
            std::string line = "  " + n;
            if (info) line += " = " + info->getter() + "  (" + info->description + ")";
    ...
    if (sep == std::string_view::npos) {
        ...
        if (val.empty() && !rex::cvar::GetFlagInfo(cmd)) {
            entries_.push_back({spdlog::level::warn, "console",
                "[console] unknown cvar: " + std::string(cmd)});
        } else {
            entries_.push_back({spdlog::level::info, "console",
                "[console] " + std::string(cmd) + " = " + val});
        }
        return;
    }
...
void ConsoleDialog::OnDraw(ImGuiIO& io) {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(0, io.DisplaySize.y - window_height), ImGuiCond_Always);
    ImGui::SetNextWindowSize(
        ImVec2(io.DisplaySize.x, window_height), ImGuiCond_Always);
    ...
    if (!ImGui::Begin("Console##reblue", &visible_, flags)) {
        ImGui::End();
        return;
    }
    ...
    for (int i = 0; i < kNumCategories; ++i) {
        ImGui::Checkbox(kCategoryNames[i], &category_filter_[i]);
        if (i < kNumCategories - 1) ImGui::SameLine();
    }

    // --- Log area ---
    const float input_height = ImGui::GetFrameHeightWithSpacing() + 4.0f;
    ImGui::BeginChild("##log", ImVec2(0, -input_height), false,
    ...
    for (auto& entry : entries_) {
        // Level filter.
        if (static_cast<int>(entry.level) < min_level_) continue;
        // Category filter.
        bool show_cat = false;
        for (int i = 0; i < kNumCategories; ++i) {
            if (category_filter_[i] && entry.category == kCategoryNames[i]) {
                show_cat = true;
                break;
            }
        }
        // "console" pseudo-category always shown.
        if (entry.category == "console") show_cat = true;
        if (!show_cat) continue;
    ...
    if (ImGui::InputText("##cmd", input_buf_, sizeof(input_buf_),
                         input_flags, InputTextCallback, this)) {
        submit = true;
    }
