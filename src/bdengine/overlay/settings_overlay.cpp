/**
 * @file        bdengine/overlay/settings_overlay.cpp
 *
 ...
 */
#include "bdengine/overlay/settings_overlay.h"
#include <rex/cvar.h>
#include <imgui.h>
#include <algorithm>
#include <string>
...
SettingsDialog::SettingsDialog(rex::ui::ImGuiDrawer* imgui_drawer,
                               std::filesystem::path config_path)
    : ImGuiDialog(imgui_drawer), config_path_(std::move(config_path)) {}

static const char* LifecycleBadge(rex::cvar::Lifecycle lc) {
    switch (lc) {
        case rex::cvar::Lifecycle::kHotReload:      return " [live]";
        case rex::cvar::Lifecycle::kRequiresRestart: return " [restart]";
        case rex::cvar::Lifecycle::kInitOnly:        return " [init-only]";
    }
    return "";
}
...
static ImVec4 LifecycleColor(rex::cvar::Lifecycle lc) {
    switch (lc) {
        case rex::cvar::Lifecycle::kHotReload:      return {0.4f, 1.0f, 0.4f, 1.0f};
        case rex::cvar::Lifecycle::kRequiresRestart: return {1.0f, 1.0f, 0.4f, 1.0f};
        case rex::cvar::Lifecycle::kInitOnly:        return {1.0f, 0.4f, 0.4f, 1.0f};
    }
...
void SettingsDialog::OnDraw(ImGuiIO& /*io*/) {
    ...
    if (!ImGui::Begin("Settings##reblue", &visible_, ImGuiWindowFlags_NoCollapse)) {
        ImGui::End();
        return;
    }
    ...
    for (auto& entry : registry) {
        ...
        if (!searching) {
            if (selected_category_ >= 0 &&
                selected_category_ < static_cast<int>(categories.size()) &&
        ...
        if (entry.type == rex::cvar::FlagType::Boolean) {
            ...
            for (int i = 0; i < static_cast<int>(opts.size()); ++i) {
                if (opts[i] == current_val) { cur_idx = i; break; }
            }
            ...
            int vmin = entry.constraints.min.has_value()
                ? static_cast<int>(*entry.constraints.min) : INT_MIN;
            int vmax = entry.constraints.max.has_value()
                ? static_cast<int>(*entry.constraints.max) : INT_MAX;
            ...
            if (ImGui::InputDouble("##v", &v, 0.0, 0.0, "%.4f")) {
                if (entry.constraints.min) v = std::max(v, *entry.constraints.min);
                if (entry.constraints.max) v = std::min(v, *entry.constraints.max);
                rex::cvar::SetFlagByName(entry.name, std::to_string(v));
            }
        } else {
        ...
        // Lifecycle badge.
        ImGui::SameLine();
        ImGui::TextColored(LifecycleColor(entry.lifecycle),
                           "%s", LifecycleBadge(entry.lifecycle));
