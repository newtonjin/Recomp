...
class XNativeFile : public rex::system::XObject {
public:
    static const rex::system::XObject::Type kObjectType =
        static_cast<rex::system::XObject::Type>(100);

    // Normal mode: wraps a std::fstream.
    XNativeFile(rex::system::KernelState* ks, std::fstream&& stream,
    ...
    XNativeFile(rex::system::KernelState* ks, std::shared_ptr<CachedBlob> blob,
                std::string path)
    ...
    XNativeFile(rex::system::KernelState* ks, std::shared_ptr<RamdiskBlob> ram,
    ...
    uint64_t position() const { return static_cast<uint64_t>(position_); }

    // Sets the current file position directly.
    void set_position(uint64_t pos) { position_ = static_cast<size_t>(pos); }

    // Returns the total file size in bytes (0 if unknown/unsupported).
    uint64_t file_size() {
        if (is_ramdisk()) return static_cast<uint64_t>(RamdiskSize());
        if (is_cached())  return static_cast<uint64_t>(cached_size());
        if (!host_path_.empty()) {
            std::error_code ec;
            auto sz = std::filesystem::file_size(host_path_, ec);
            return ec ? 0 : sz;
        }
        return 0;
    }

    // Returns true if the path is a directory (stream/ramdisk blobs never are).
    bool is_dir() const {
        if (host_path_.empty()) return false;
        std::error_code ec;
        return std::filesystem::is_directory(host_path_, ec);
    }
...
class XNativeFind : public rex::system::XObject {
public:
    static const rex::system::XObject::Type kObjectType =
        rex::system::XObject::Type::Enumerator;

    XNativeFind(rex::system::KernelState* ks,
