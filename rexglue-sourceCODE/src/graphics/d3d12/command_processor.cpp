...
void D3D12CommandProcessor::IssueSwap(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
  ...
  presenter->RefreshGuestOutput(
      uint32_t(swap_texture_desc.Width), uint32_t(swap_texture_desc.Height), 1280, 720,
...
bool D3D12CommandProcessor::IssueCopy() {
  ...
  if (REXCVAR_GET(d3d12_readback_resolve) && !texture_cache_->IsDrawResolutionScaled() &&
      written_length) {
    ...
    if (readback_buffer != nullptr) {
      ...
      SubmitBarriers();
      ID3D12Resource* shared_memory_buffer = shared_memory_->GetBuffer();
      deferred_command_list_.D3DCopyBufferRegion(readback_buffer, 0, shared_memory_buffer,
      ...
      if (AwaitAllQueueOperationsCompletion()) {
        ...
        void* readback_mapping;
        if (SUCCEEDED(readback_buffer->Map(0, &readback_range, &readback_mapping))) {
          std::memcpy(memory_->TranslatePhysical(written_address), readback_mapping,
                      written_length);
          D3D12_RANGE readback_write_range = {};
          readback_buffer->Unmap(0, &readback_write_range);
        }
...
bool D3D12CommandProcessor::UpdateBindings(const D3D12Shader* vertex_shader,
  ...
  // NOTE(umvc3-recomp): MT Framework writes non-standard values to these regs.
  // Changed from assert_true to warn to avoid aborting the CP thread.
  if (regs[XE_GPU_REG_SQ_VS_CONST] != 0x000FF000 &&
      regs[XE_GPU_REG_SQ_VS_CONST] != 0x00000000) {
    REXGPU_WARN("SQ_VS_CONST unexpected: {:08X} (expected 000FF000 or 0)",
                regs[XE_GPU_REG_SQ_VS_CONST]);
  }
  if (regs[XE_GPU_REG_SQ_PS_CONST] != 0x000FF100 &&
      regs[XE_GPU_REG_SQ_PS_CONST] != 0x00000000) {
    REXGPU_WARN("SQ_PS_CONST unexpected: {:08X} (expected 000FF100 or 0)",
